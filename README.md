...
Ok, so we solve this by wrapping our state in a function.
...
Ok, now this code almost works. Call `createStore`, and set the returned store equal to a variable. Then change our remaining code to make sure we are properly referencing the `dispatch` method. We can use the almost-working code in the following manner.
...
Ok. Now we can get our code working by changing render to the following:
...
Our code is back to working. And it looks like we have function called createStore which can work, with any JavaScript application...almost.

## Abstract away the reducer

We know that Redux works by an action being dispatched which calls a reducer and then renders the view. And our createStore's dispatch method does that.
...
However, notice that we did not move the changeCount reducer into the createStore function. Take a look at it. This code is particular to our application. 
...
We happen to have an application that increases a count. But we can imagine applications that manage people's songs, their github repositories, their contacts. So we want our dispatch method to call a reducer every time an action is dispatched. However, we don't want `createStore` function, which we want to be generic enough for any JavaScript application, to specify what that reducer is, or what it does. So we make the reducer an argument to our `createStore` function. Then we pass through our reducer function when invoking the createStore method. 
